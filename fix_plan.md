...
- [x] Per-response actions (copy / feedback / regenerate): each assistant reply renders a Claude-style action row — copy-to-clipboard, thumbs up/down + detail dialog, regenerate. Feedback via `POST /api/feedback` → `data/feedback.json`. See refactor + feedback dialog entries above.
