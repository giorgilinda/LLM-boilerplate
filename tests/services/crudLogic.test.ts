...
describe("crudLogic", () => {
  ...
  describe("buildListUrl", () => {
    it("builds absolute URL with query params", () => {
      const url = buildListUrl("https://api.test/items", { status: "active", q: "test" });
  ...
  describe("fetchList", () => {
    it("returns plain array when no parser configured", async () => {
      const fetchSpy = jest
        .spyOn(global, "fetch")
        .mockResolvedValue(
          new Response(JSON.stringify([{ id: 1, name: "A" }] as MockEntity[]), { status: 200 })
        );
    ...
    it("uses parseListResponse when configured (preserves meta)", async () => {
      ...
      expect(data).toEqual({ list: [{ id: 1, name: "WithMeta" }], info: { count: 1 } });
    });
  ...
  describe("fetchItem", () => {
    it("returns parsed JSON on success", async () => {
      jest
        .spyOn(global, "fetch")
        .mockResolvedValue(new Response(JSON.stringify({ id: 1, name: "A" }), { status: 200 }));
    ...
    it("throws notFoundMessage when non-OK", async () => {
      ...
      await expect(
        fetchItem<MockEntity>(
          { entityKey: "items", baseUrl: "/api/items", notFoundMessage: "Item missing" },
          1
        )
  ...
  describe("request contracts", () => {
    it("createItem sends POST with JSON body + content-type", async () => {
      ...
      await createItem<MockEntity>({ entityKey: "items", baseUrl: "/api/items" }, { name: "A" });
    ...
    it("updateItem sends PATCH to /:id with JSON body + content-type", async () => {
      const fetchSpy = jest
        .spyOn(global, "fetch")
        .mockResolvedValue(new Response(JSON.stringify({ id: 1, name: "B" }), { status: 200 }));
    ...
    it("deleteItem sends DELETE to /:id", async () => {
      ...
      await deleteItem<MockEntity>({ entityKey: "items", baseUrl: "/api/items" }, 1);

      expect(fetchSpy).toHaveBeenCalledWith("/api/items/1", { method: "DELETE" });
    });
  });
});
